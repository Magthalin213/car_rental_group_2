...
public class CustomerDaoImpl {
	...
	public List<CustomerVehicleDto> fetchRecord() {
		...
		String sql = "select *" + "from customer c ,lease l,vehicle v  " + "where c.id=l.customer_id and "
	...
	public List<CustomerDto> getById(int cuid) throws SQLException {
		...
		String sql = "select c.id,c.first_name,c.last_name,c.email,c.phone_number,"
				+ "l.vehicle_id,l.start_date,l.end_date,l.type " + "from customer c join lease l on l.customer_id=c.id "
				+ "where c.id=?";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setInt(1, cuid);
		ResultSet rst = pstmt.executeQuery();
	...
	public List<CustomerVehicleDto> fetchDet(int id1) {
		...
		String sql = "select *" + "from customer c join lease l " + "on c.id=l.customer_id join "
